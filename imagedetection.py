...
##time.sleep(0)
firstFrame=None
area=500 #threshold value of the size of the object
while True:
    ...
    if firstFrame is None:
        firstFrame=gaussianImg #capturing the initial frame
        continue
    imgDiff=cv2.absdiff(firstFrame,gaussianImg) #absolute difference
    threshImg=cv2.threshold(imgDiff,25,255,cv2.THRESH_BINARY) [1]
    threshImg=cv2.dilate(threshImg,None,iterations=2) #left over-erotion
    cnts=cv2.findContours(threshImg.copy(),cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
    cnts=imutils.grab_contours(cnts)
    for c in cnts:
        if cv2.contourArea(c) < area : #check whether the area is greater than threshold area
            continue
